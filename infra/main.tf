...
# Public subnets (NLB, internet gateway)
...
# Private subnets (RDS, future workloads)
...
# Nodes SG: allow all within the VPC; the node group itself adds the EKS-required rules.
...
# RDS SG: only allow PostgreSQL from the node SG
...
resource "aws_eks_node_group" "main" {
  ...
  subnet_ids      = aws_subnet.public[*].id  # public subnets (no NAT gateway in this blueprint)
...
resource "aws_db_instance" "postgres" {
  ...
  # Single-AZ at Tier 1; enable multi_az for HA (Tier 2 enhancement)
  multi_az               = false
